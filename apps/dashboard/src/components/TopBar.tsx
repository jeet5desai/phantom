...
export default function TopBar() {
  ...
  if (!isLoaded) return <header className="h-16 border-b border-border bg-surface sticky top-0 z-50 animate-pulse" />;
          ...
          {showNotifications && (
            <div className="absolute top-12 right-0 w-80 glass z-50 overflow-hidden fade-in shadow-2xl">
              <div className="p-4 border-b border-border flex justify-between items-center bg-background/50 backdrop-blur-sm">
                ...
                <button className="text-xs font-bold text-accent-primary hover:underline transition-all">Mark all read</button>
              </div>
        ...
        <div className="relative" ref={userMenuRef}>
          ...
          >
            ...
            <div className="hidden sm:flex flex-col items-start gap-0">
              <span className="text-sm font-semibold text-text-primary leading-tight">{user?.firstName || "Guest"}</span>
              <span className="text-[10px] text-text-tertiary font-medium uppercase tracking-wider">{user?.publicMetadata?.role as string || "Developer"}</span>
            </div>
            <ChevronDown size={14} className={`text-text-tertiary transition-transform duration-300 ${showUserMenu ? 'rotate-180' : ''}`} />
          </button>
            ...
            <div className="absolute top-12 right-0 w-64 glass z-50 overflow-hidden zoom-in-95 shadow-2xl">
              ...
              <div className="p-4 bg-background/50 backdrop-blur-md border-b border-border flex items-center gap-3">
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  <span className="text-[11px] text-text-tertiary truncate">{user?.primaryEmailAddress?.emailAddress}</span>
                </div>
              </div>
              ...
              <div className="p-2">
                ...
                >
                  <Settings size={16} className="text-text-tertiary group-hover:text-accent-primary transition-colors" />
                ...
                <Link 
                  ...
                  onClick={() => setShowUserMenu(false)}
                >
                  <Shield size={16} className="text-text-tertiary group-hover:text-accent-primary transition-colors" />
                ...
                >
                  <LogOut size={16} className="text-error/70 group-hover:text-error transition-colors" />
              ...
              <div className="px-4 py-3 bg-accent-light/30 border-t border-border flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold text-accent-primary uppercase tracking-widest">Development mode</span>
                </div>
