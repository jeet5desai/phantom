...
const NOTIFICATIONS = [
  ...
  },
  {
    ...
    message: "Agent 'customer-support' is attempting to access sensitive data in 'Customer DB' without a valid session token.",
    time: "15m ago",
    type: "warning",
    isRead: false
  },
  {
    ...
    message: "Your 'Stripe Production Key' is set to expire in 3 days. Please rotate it to avoid service interruption.",
    time: "2h ago",
    type: "error",
    isRead: true
  },
  {
    id: 4,
    title: "Policy Update",
    message: "Global policy 'Production Access' has been updated to require Multi-Factor Authentication.",
    time: "1d ago",
    type: "info",
    isRead: true
  },
  {
    ...
    message: "Your weekly security audit is ready. 2,402 actions were verified with a 100% compliance rate.",
    time: "2d ago",
    type: "success",
    isRead: true
  }
    ...
    <div className="flex flex-col gap-lg fade-in">
      <div className="flex justify-between items-end mb-2">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-text-secondary text-lg">Stay updated with agent activity, security alerts, and system status.</p>
        </div>
        ...
        {NOTIFICATIONS.map((notif) => (
          ...
          >
            ...
            <div className={`w-12 h-12 rounded-full shrink-0 flex items-center justify-center ${
              notif.type === 'info' ? 'bg-accent-light text-accent-primary' :
              notif.type === 'warning' ? 'bg-warning-bg text-warning' :
              notif.type === 'error' ? 'bg-error-bg text-error' :
            ...
            <div className="flex flex-col gap-1 flex-1">
              <div className="flex justify-between items-start">
                <h3 className={`font-display font-bold text-lg ${!notif.isRead ? 'text-text-primary' : 'text-text-secondary'}`}>
                  {notif.title}
                </h3>
              ...
              {!notif.isRead && (
                <div className="flex gap-4 mt-3">
                  <button className="text-[11px] font-bold text-accent-primary hover:underline uppercase tracking-wider">Mark as read</button>
                  <button className="text-[11px] font-bold text-text-tertiary hover:text-text-primary hover:underline uppercase tracking-wider">Dismiss</button>
                </div>
              )}
            </div>
