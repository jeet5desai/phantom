...
const RESOURCES = [
  {
    ...
    description:
      'Learn how to deploy your first agent and configure identity in less than 5 minutes.',
    icon: Zap,
      ...
      <div className="bg-accent-primary rounded-2xl p-lg lg:p-16 text-white flex flex-col items-center text-center gap-8 relative overflow-hidden">
        ...
        <div className="flex flex-col gap-4 relative z-10">
          ...
          <p className="text-white/80 text-lg max-w-3xl">
            Search our documentation, tutorials, and developer resources to build secure AI agents.
          </p>
        </div>
        ...
        <div className="w-full max-w-2xl relative z-10">
          <Search
            size={20}
            className="absolute left-6 top-1/2 -translate-y-1/2 text-text-tertiary"
          />
          <input
            type="text"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-lg -mt-12 relative z-20 px-4">
        {RESOURCES.map((res) => {
          ...
          return (
            <div
              key={res.title}
              className="glass p-lg flex flex-col gap-4 hover:-translate-y-2 transition-all duration-300 cursor-pointer group "
            >
              <div
                className={`w-12 h-12 ${res.bgColor} ${res.color} rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform`}
              >
                <Icon size={24} />
              </div>
            ...
            <div className="p-lg grid grid-cols-1 md:grid-cols-2 gap-md">
              <div className="flex flex-col gap-3 group cursor-pointer">
                <div className="aspect-video bg-background border border-border rounded-xl flex items-center justify-center relative overflow-hidden">
                  ...
                  />
                  <div className="absolute bottom-3 right-3 px-2 py-1 bg-black/60 rounded text-[10px] font-bold text-white">
                    4:20
                  </div>
                </div>
                <span className="font-bold text-sm group-hover:text-accent-primary transition-colors">
                  Understanding Agent Sandboxing
                </span>
              </div>
              <div className="flex flex-col gap-3 group cursor-pointer">
                <div className="aspect-video bg-background border border-border rounded-xl flex items-center justify-center relative overflow-hidden">
          ...
          <div className="glass p-lg flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2">
              {POPULAR_DOCS.map((doc) => (
                <div
                  key={doc}
                  className="flex items-center justify-between p-3 hover:bg-background rounded-lg transition-colors cursor-pointer group"
                >
