...
export default function VaultPage() {
  ...
  const fetchCredentials = async () => {
    setLoading(true);
    const data = await apiRequest('GET', '/api/v1/credentials');
    if (data?.credentials) {
  ...
  const handleAddSecret = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    const result = await apiRequest('POST', '/api/v1/credentials', {
  ...
  const handleDelete = async (credentialId: string, serviceName: string) => {
    const confirmed = window.confirm(`Delete credential for "${serviceName}"? This cannot be undone.`);
    if (!confirmed) return;

    const result = await apiRequest('DELETE', `/api/v1/credentials/${credentialId}`);
    if (result) {
      fetchCredentials();
    }
  };
    ...
    <div className="flex flex-col gap-lg fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-md">
        ...
        <div className="glass p-lg flex flex-col gap-2">
          ...
          <span className="text-3xl font-display font-bold text-success">{activeCount > 0 ? 'A+' : '—'}</span>
        </div>
      </div>
      ...
      <div className="glass overflow-hidden flex flex-col">
        ...
        {loading && credentials.length === 0 ? (
          ...
          <div className="p-lg flex flex-col items-center gap-4 py-16">
            ...
            <p className="text-text-secondary text-sm">No credentials stored yet. Add your first secret to get started.</p>
          </div>
        ) : (
                ...
                {credentials.map((cred) => {
                  ...
                  return (
                    <tr key={cred.id} className="group hover:bg-background transition-colors">
                      <td className="px-lg py-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-col">
                            <span className="font-bold text-text-primary">{cred.label || cred.service}</span>
                      ...
                      <td className="px-lg py-5">
                        <span className="text-sm text-text-secondary">{formatTime(cred.created_at)}</span>
                      </td>
                      <td className="px-lg py-5">
      ...
      <Dialog isOpen={showAddModal} onClose={() => setShowAddModal(false)}>
        <DialogContent>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              Securely store credentials for your agents. Secrets are encrypted at rest using AES-256-GCM.
            </DialogDescription>
          </DialogHeader>
